...
exports.createTroops = async (game_id) => {
    const troopData = [
    ...
    try {
        await Promise.all(troopData.map(troopData => new Troop(troopData).save()));
    } catch (err) {
        console.error('Fehler beim Erstellen der Troops:', err);
    }
};

exports.getTroops = async (game_id) => {
    try {
        const troops = await Troop.find({ 
            game: game_id
        }).sort({ 'index': 1 });
        return troops.length ? troops : null;
    } catch (err) {
        console.error('Fehler beim Abrufen der Troops:', err);
        return null;
    }
};

exports.deleteTroops = (game_id) => {
    return Troops.deleteMany({ game: game_id })
        .then(deletionResult => {
            if (deletionResult.deletedCount > 0) {
                return true;
            } else {
                console.log('Keine Troops zum Löschen gefunden.');
                return false; 
            }
        })
        .catch(err => {
            console.error('Fehler beim Löschen der Troops:', err);
            throw err;
        });
};

exports.deleteTroopsByGameId = async (gameId) => {
    return await Troop.deleteMany({ game: gameId });
};